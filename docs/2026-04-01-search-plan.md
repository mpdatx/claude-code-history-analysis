...
**Goal:** Add a `search` command that performs full-text regex search across all Claude Code session history and displays results as timeline-style session cards with match highlighting.

**Architecture:** New `tools/search.py` module with three functions: `scan_sessions()` for JSONL scanning and regex matching, `generate_html()` for the highlighted HTML report, and `generate_markdown()` for terminal output. A small `search` subcommand is added to `tools/cli.py` following the same pattern as the existing `timeline` command.

**Tech Stack:** Python 3.8+, `re` module for regex, existing `claude_history` library (`parsing`, `filters`, `timestamps`, `html_theme`), `typer` for CLI.
...
|------|--------|----------------|
| `tools/search.py` | Create | Scanning, matching, HTML generation, markdown generation |
| `tools/cli.py` | Modify (add ~30 lines) | `search` subcommand wiring |

---

### Task 1: scan_sessions() — JSONL scanning and regex matching

**Files:**
- Create: `tools/search.py`

This is the core function. It takes a directory, a compiled regex, and optional `recent_days`, scans all JSONL files, and returns a list of session dicts (only sessions with matches).

- [ ] **Step 1: Create `tools/search.py` with `scan_sessions()`**
            ...
            for item in content:
                if isinstance(item, dict):
                    if item.get("type") == "tool_result":
                        ...
                        spans = _find_matches(pattern, result_text)
...
Run: `python -c "from tools.search import scan_sessions; print('OK')"`
...
```bash
git add tools/search.py
git commit -m "feat(search): add scan_sessions for JSONL regex matching"
```

---

### Task 2: generate_html() — HTML report with match highlighting

**Files:**
- Modify: `tools/search.py`

Add `generate_html()` that takes the result from `scan_sessions()` and the pattern string, and returns a complete HTML page.

- [ ] **Step 1: Add `_highlight_text()` helper to `tools/search.py`**

Append after `_find_matches`:
...
- [ ] **Step 2: Add `generate_html()` to `tools/search.py`**

Append after `_highlight_text`:
...
- [ ] **Step 3: Add `_build_heatmap()` to `tools/search.py`**

Append after `generate_html`:
...
- [ ] **Step 4: Verify HTML generation imports cleanly**

Run: `python -c "from tools.search import scan_sessions, generate_html; print('OK')"`
...
```bash
git add tools/search.py
git commit -m "feat(search): add HTML report generation with match highlighting"
```

---

### Task 3: generate_markdown() — terminal/markdown output

**Files:**
- Modify: `tools/search.py`

- [ ] **Step 1: Add `generate_markdown()` to `tools/search.py`**

Append after `_build_heatmap`:
...
Run: `python -c "from tools.search import generate_markdown; print('OK')"`
...
```bash
git add tools/search.py
git commit -m "feat(search): add markdown/terminal output"
```

---

### Task 4: CLI wiring — add `search` subcommand to `tools/cli.py`

**Files:**
- Modify: `tools/cli.py`

- [ ] **Step 1: Add the `search` command to `tools/cli.py`**

Add after the `report` command (before `if __name__ == "__main__":`):
...
- [ ] **Step 2: Test the CLI wiring**

Run: `python -m claude_history search --help`

Expected output should show:
```
Usage: python -m claude_history search [OPTIONS] PATTERN

  Full-text regex search across session history.

Arguments:
  PATTERN  Regex search pattern (case-insensitive) [required]
...
- [ ] **Step 3: Run a real search to verify end-to-end**

Run: `python -m claude_history search "authentication" --recent 30`

Expected: Either "No matches found." or a file path to `output/search_authentication.html`. Open the HTML file in a browser and verify:
- Stats grid shows match counts
- Session cards show project badges, match count badges
- Expanding a card shows events with yellow `<mark>` highlights
- "Prev/Next" match navigation buttons scroll between highlights
- Filter bar works

- [ ] **Step 4: Test markdown output**

Run: `python -m claude_history --no-html search "authentication" --recent 30`

Expected: A `.md` file with sessions grouped, matched events shown with `**bold**` highlights.

- [ ] **Step 5: Test regex pattern**

Run: `python -m claude_history search "auth(entication|orization)" --recent 30`

Expected: Matches both "authentication" and "authorization" occurrences.

- [ ] **Step 6: Test invalid regex**

Run: `python -m claude_history search "[invalid"`

Expected: `Invalid regex pattern: ...` error message, exit code 1.

- [ ] **Step 7: Commit**

```bash
git add tools/cli.py
git commit -m "feat(search): add search subcommand to CLI"
```
...
- [ ] **Step 1: Add `search` to the commands table in `README.md`**

In the Commands table, add a row after `report`:

```markdown
| `search <pattern>` | Full-text regex search across all session history |
```

- [ ] **Step 2: Add a search example to the Examples section**
...
```bash
git add README.md
git commit -m "docs: add search command to README"
```
